...
| 步骤 | 问题 | 填什么 |
|---|---|---|
| [1/5] | 玩家连接端口（TCP） | 玩家在游戏里「添加服务器」填的端口，默认 19132。面板只给了特定 TCP 端口就填那个 |
| [2/5] | BDS 信令地址 | 默认 `127.0.0.1:19132`（代理和 BDS 同机）。BDS 在别的机器填那台机器的 `IP:端口`，向导会自动检测连通性 |
| [3/5] | 游戏流量端口（UDP） | 所有玩家共用的 UDP 端口，默认 19133。面板只给了特定 UDP 端口就填那个 |
| [4/5] | 公网 IP | 玩家实际能访问到的 IP。向导自动检测；检测出 192.168.x.x / 10.x.x.x 说明是内网（局域网联机没问题），公网/面板部署改填公网 IP |
| [5/5] | 对外通告的 UDP 端口 | 面板/NAT 把外部端口映射到不同内部端口才需要改，一般直接回车 |
...
```properties
transport=nethernet
server-port=19132          # BDS 自己的端口，只给本代理访问
server-udp-ports=          # 留空！由代理统一复用，切勿填端口
```
