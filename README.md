...
**进服报错"Door"（多人联机连接错误），且 BDS 控制台毫无日志？**
九成是 BDS 的 `server.properties` 里配了 `server-udp-ports=19132`（固定单端口，部分面板模板会预置）。表现：第一次连接正常，断开后**重连必失败**（旧的僵尸连接占着端口，新连接绑定失败变成死会话），第二个玩家也进不来。解决办法：注释掉 `server-udp-ports`（`#server-udp-ports=`）并**重启 BDS**。本代理不需要 BDS 固定端口——BDS 内部给每个玩家分配独立临时端口即可，反正只有代理访问它。v1.0.3 起代理会在日志里自动点破这种配置（"BDS 游戏端口与信令端口同为 xxx"）。

**进服报错"Spyglass"？**
账号权限问题，与代理无关：到 [account.xbox.com](https://account.xbox.com) → 隐私和在线安全，把"加入多人游戏"设为允许；家庭组未成年账号需家长放行。（另注意服务端白名单也会导致进不去。）
